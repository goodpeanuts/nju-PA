...
#define shamtJ() do { *imm = BITS(i, 25, 20);} while(0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    case TYPE_R: src1R(); src2R(); shamtJ(); break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm; ftrace(call, s->pc, s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, paddr_t t = s->pc + 4; s->dnpc = (src1 + imm) & ~1; R(rd) = t;  ftrace(ret, s->pc, s->dnpc));
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(rd) = (unsigned long long)src1 * src2 >> 32); 
  ...
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , R, R(rd) = src1 << imm);
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , R, R(rd) = ((int32_t)src1 >> imm));
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(rd) = src1 >> src2);
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , R, R(rd) = src1 >> imm);
